...
public class GrantUrlTesterService(IHttpClientFactory factory, ILogger<IGrantUrlTesterService> logger) : IGrantUrlTesterService
{
    public async Task<bool> TestGrantUrl(string urlHook, string url, string token)
    {
        ...
        msg.Headers.Add("X-schoolapp-whtoken", token);
        ...
        try
        {
            ...
            var tokenReceived = response.Headers.TryGetValues("X-schoolapp-whtoken", out var tokenValues) ? tokenValues.FirstOrDefault() : null;
